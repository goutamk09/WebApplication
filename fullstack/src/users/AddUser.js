...
export default function AddUser() {
    ...
    //4.Post data When clicked on submit by using Axios
    const onSubmit = async (e) => {
        //To prevent showing feilds and values in url when we click on submit use preventDefault
        e.preventDefault();

        //here along with path we need to pass the user object from 1st line
        await axios.post("http://localhost:8080/save", user);

        //after posting data to database..it should navigate to home page for tht use "useNavigate hook" in 1st line
        navigate("/");
    }
